...
class TemplatePagingSource @AssistedInject internal constructor(
    @Assisted private val useCaseParams: GetTemplatesUseCase.Params,
    @Assisted private val getTemplatesUseCase: GetTemplatesUseCase,
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Template> {
        ...
        val newUseCaseParams = useCaseParams.copy(page = position)

        return try {
            val loadData = getTemplatesUseCase.run(newUseCaseParams)
            LoadResult.Page(
    ...
    interface Factory {
        fun create(
            useCaseParams: GetTemplatesUseCase.Params,
            getTemplatesUseCase: GetTemplatesUseCase
        ): TemplatePagingSource
    }
